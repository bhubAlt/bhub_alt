...
function M.init(Rayfield, beastHubNotify, Window, myFunctions, beastHubIcon, equipItemByName, equipItemByNameV2, getMyFarm, getFarmSpawnCFrame, getAllPetNames, sendDiscordWebhook, allSeedsData, allSeedsOnly, equipFruitById)
    ...
    --fav/unfav fruits
    Plants:CreateSection("Fruits")
    -- Auto Favorite Fruits
    local autoFavFruitsEnabled
    local autoFavFruitsThread
    local dropdown_selectedFruitForAutoFavUnfav = Plants:CreateDropdown({
        Name = "Select Fruit",
        Options = allSeedsOnly,
        CurrentOption = {},
        MultipleOptions = false,
        Flag = "selectedFruit_autoFavUnfav", -- A flag is the identifier for the configuration file, make sure every element has a different flag if you're using configuration saving to ensure no overlaps
        Callback = function(Options)
        end,
    })
    local searchDebounce_seedForFavUnfav = nil
    Plants:CreateInput({
        Name = "Search fruit",
        PlaceholderText = "fruit",
        RemoveTextAfterFocusLost = false,
        Callback = function(Text)
            if searchDebounce_seedForFavUnfav then
                task.cancel(searchDebounce_seedForFavUnfav)
            end
            searchDebounce_seedForFavUnfav = task.delay(0.5, function()
                local results = {}
                local query = string.lower(Text)
                ...
                dropdown_selectedFruitForAutoFavUnfav:Refresh(results)
                dropdown_selectedFruitForAutoFavUnfav:Set(dropdown_selectedFruitForAutoFavUnfav.Options[1]) --set to current selected
    ...
    Plants:CreateButton({
        Name = "Clear fruit",
        Callback = function()
            dropdown_selectedFruitForAutoFavUnfav:Set({"None"})
        end,
    })
    ...
    Plants:CreateToggle({
        ...
        Flag = "autoFavFruits",
        Callback = function(Value)
            autoFavFruitsEnabled = Value

            if autoFavFruitsEnabled then
                if autoFavFruitsThread then
                    return
                end

                autoFavFruitsThread = task.spawn(function()
                    local player = game:GetService("Players").LocalPlayer
                    local rs = game:GetService("ReplicatedStorage")
                    local favEvent = rs.GameEvents.Favorite_Item
                    local backpack = player:WaitForChild("Backpack")

                    while autoFavFruitsEnabled do
                        if backpack then
                            for _, item in ipairs(backpack:GetChildren()) do
                                local attrB = item:GetAttribute("b")
                                local attrD = item:GetAttribute("d")
                                local curFruitName = item:GetAttribute("f")
                                if dropdown_selectedFruitForAutoFavUnfav.CurrentOption[1] and dropdown_selectedFruitForAutoFavUnfav.CurrentOption[1] ~= "None" then
                                    if  curFruitName == dropdown_selectedFruitForAutoFavUnfav.CurrentOption[1] and attrB == "j" and attrD == false then
                                        favEvent:FireServer(item)
                                    end
                                else
                                    if attrB == "j" and attrD == false then
                                        favEvent:FireServer(item)
                                    end
                                end
                            end
                    ...
                    autoFavFruitsThread = nil
                end)

            else
                autoFavFruitsEnabled = false
                autoFavFruitsThread = nil
            end
        end,
    })

    -- Auto Unfavorite Fruits
    local autoUnfavFruitsEnabled
    local autoUnfavFruitsThread
    Plants:CreateToggle({
        Name = "Auto Unfavorite Fruits",
        CurrentValue = false,
        Flag = "autoUnfavFruits",
        Callback = function(Value)
            autoUnfavFruitsEnabled = Value

            if autoUnfavFruitsEnabled then
                if autoUnfavFruitsThread then
                    return
                end

                autoUnfavFruitsThread = task.spawn(function()
                    local player = game:GetService("Players").LocalPlayer
                    local rs = game:GetService("ReplicatedStorage")
                    local unfavEvent = rs.GameEvents.Favorite_Item
                    local backpack = player:WaitForChild("Backpack")

                    while autoUnfavFruitsEnabled do
                        if backpack then
                            for _, item in ipairs(backpack:GetChildren()) do
                                local attrB = item:GetAttribute("b")
                                local attrD = item:GetAttribute("d")
                                local curFruitName = item:GetAttribute("f")
                                if dropdown_selectedFruitForAutoFavUnfav.CurrentOption[1] and dropdown_selectedFruitForAutoFavUnfav.CurrentOption[1] ~= "None" then
                                    if curFruitName == dropdown_selectedFruitForAutoFavUnfav.CurrentOption[1] and attrB == "j" and attrD == true then
                                        unfavEvent:FireServer(item)
                                    end
                                else
                                    if attrB == "j" and attrD == true then
                                        unfavEvent:FireServer(item)
                                    end
                                end
                    ...
                    autoUnfavFruitsThread = nil
                end)

            else
                autoUnfavFruitsEnabled = false
                autoUnfavFruitsThread = nil
            end
        end,
    })
